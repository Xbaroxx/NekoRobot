...
from NekoRobot import BOT_ID
from NekoRobot.conf import get_int_key, get_str_key

# from NaoRobot.db.mongo_helpers.nsfw_guard import add_chat, get_all_nsfw_chats, is_chat_in_db, rm_chat
from NekoRobot.pyrogramee.telethonbasics import is_admin
from NekoRobot.events import register
from NekoRobot import MONGO_DB_URI 
from pymongo import MongoClient
from NekoRobot.modules.sql.nsfw_watch_sql import (
    add_nsfwatch,
...
async def ws(event):
    ...
    if nude.is_nude("./nudes.jpg"):
        ...
        hh = sender.id
        final = f"**NSFW DETECTED**\n\n{st}](tg://user?id={hh}) your message contain NSFW content.. So, Nao deleted the message\n\n **Nsfw Sender - User / Bot :** {st}](tg://user?id={hh})  \n\n`⚔️Automatic Detections Powered By NaoAI` \n**#GROUP_GUARDIAN** "
...
async def nsfw_watch(client, message):
    ...
    if not hot:
        ...
        await client.send_message(
            message.chat.id,
            f"**NSFW DETECTED**\n\n{hehe}'s message contain NSFW content.. So, Nao deleted the message\n\n **Nsfw Sender - User / Bot :** `{Escobar}` \n**Chat Title:** `{ctitle}` \n\n`⚔️Automatic Detections Powered By NaoAI` \n**#GROUP_GUARDIAN** ",
        )
        message.continue_propagation()
"""
...
__help__ = """
<b> Group Guardian: </b>
✪ NokoRobot can protect your group from NSFW senders, Slag word users and also can force members to use English
