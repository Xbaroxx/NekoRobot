...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        import importlib
        import NekoRobot.events

        path = Path(f"NekoRobot/modules/{shortname}.py")
        name = "NekoRobot.modules.{}".format(shortname)
        ...
        path = Path(f"NaoRobot/modules/{shortname}.py")
        name = "NaoRobot.modules.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        mod.register = register
        mod.NekoRobot = NekoRobot
        mod.tbot = telethn
        mod.logger = logging.getLogger(shortname)
        spec.loader.exec_module(mod)
        sys.modules["NekoRobot.modules." + shortname] = mod
        print("Successfully imported " + shortname)


path = "NekoRobot/modules/*.py"
